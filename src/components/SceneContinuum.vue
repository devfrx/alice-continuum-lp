...
const cells = [
  ...
  { k: 'Vector search',       v: 'pgvector search over local content' },
  { k: 'Realtime',            v: 'Y.js CRDT for live editing' },
  { k: 'Assistant hooks',     v: 'AL\CE can read selected context' },
  { k: 'Open files',          v: 'Markdown you can inspect outside the app' },
]
  ...
  >
    <header class="head">
      ...
      <p class="lede">
        ...
        AL\CE can query. The files stay readable outside the app.
      </p>
    </header>
...
/* Head */
.head { grid-area: head; max-width: 56rem; display: grid; gap: 1.25rem; }
.mono { margin: 0; opacity: var(--in, 0); transform: translateY(calc((1 - var(--in, 0)) * -8px)); transition: opacity 0.5s var(--ease-cinema); }
.title { margin: 0; font-size: var(--t-h1); color: var(--text-primary); display: grid; gap: 0.04em; }
.row { display: block; }
.sr-gap {
  position: absolute;
  width: 1px;
...
.r1 { opacity: calc(min(1, var(--in, 0) * 2));        transform: translateY(calc((1 - min(1, var(--in, 0) * 2))        * 60px)); transition: transform 0.9s var(--ease-cinema), opacity 0.5s var(--ease-cinema); }
.r2 { opacity: calc(min(1, var(--in, 0) * 2 - 0.3));  transform: translateY(calc((1 - min(1, var(--in, 0) * 2 - 0.3)) * 60px)); transition: transform 0.9s var(--ease-cinema), opacity 0.5s var(--ease-cinema); color: var(--text-secondary); }
.r3 { opacity: calc(min(1, var(--in, 0) * 2 - 0.6));  transform: translateY(calc((1 - min(1, var(--in, 0) * 2 - 0.6)) * 60px)); transition: transform 0.9s var(--ease-cinema), opacity 0.5s var(--ease-cinema); }
.r3 .accent { color: var(--accent); }
...
.nodes circle.center { fill: var(--text-primary); }
...
@media (max-width: 980px) {
  ...
  .constellation { position: static; transform: none; max-width: 320px; }
